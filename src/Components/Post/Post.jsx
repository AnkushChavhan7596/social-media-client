...
const Post = ({ postData, currentActiveUser }) => {
  ...
  const loadActivePostUser = async () => {
    const res = await axios.post(
      `https://social-media-ankush.herokuapp.com/get_user_by_id/${postData.authorID}`
    );
  ...
  return (
    <>
      <div className="Post">
        ...
        <Link
          to={
            postData.authorID === currentActiveUser.id
              ? "/profile"
              : `/profile/${postData.authorID}`
          }
          onClick={reload}
        >
        ...
        <div className="post_body">
          <img
            src={`https://social-media-ankush.herokuapp.com/public/Images/${postData.postImage}`}
            alt="profile"
          />
        </div>
          ...
          <div className="post_footer_left">
            <div className="icons">
              <Link
                to="/"
                onClick={() => {
                  handleLike(currentActiveUser._id, postData._id);
                }}
              >
            ...
            <Link to={`/comments/${postData._id}`}>
              <p>
                <span className="comment_count">
                  {
                    comments.filter(
                      (comment) => comment.postID === postData._id
                    ).length
                  }
                </span>
                comments
