...
const Comment = ({ comment, postID, commentsCount }) => {
    ...
    const [commentDeleteModalStyle, setCommentDeleteModalStyle] = useState("hide_comment_delete_modal");
            ...
            <div className="comment">
                ...
                <div className="comment_profile">
                    <Link to={`/profile/${commentUser._id}`}>
                       <img src={commentUser.profileImg ? `${process.env.REACT_APP_IMAGE_PATH}${commentUser.profileImg}` : `${process.env.REACT_APP_IMAGE_PATH}user (1).png`} alt="profile" />
                   </Link>
                </div>
                    ...
                    activeUser._id === comment.authorID ?
                        ...
                        <div className={`comments_delete_modal ${commentDeleteModalStyle}`}>
                            ...
                            <div className="comment_delete_btns_div">
                                <Link to={`/comments/${postID}`} className="delete_comment_link" onClick={() => { handleDeleteComment(comment._id) }}>Delete</Link>

                                <Link to={`/comments/${postID}`} className="delete_comment_link" onClick={handleCommentDeleteModal}>Cancel</Link>
                            </div>
                        </div>
                ...
                <i className="fas fa-ellipsis-v comment_dots" onClick={handleCommentDeleteModal}></i>
            </div>
