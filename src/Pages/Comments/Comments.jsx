...
const Comments = () => {
  ...
  useEffect(() => {
    // loadCommentPost();
    axios
      .post(`https://social-media-ankush.herokuapp.com/get_post_by_id`, {
        id: id,
      })
      .then((res) => {
        if (res.status === 200) {
  ...
  const handleLike = async (authorID, postID) => {
    try {
      ...
      const res = await axios.post(
        `https://social-media-ankush.herokuapp.com/post-like/${id}`,
        { postID: postID }
      );
  ...
  const loadComments = async () => {
    try {
      const res = await axios.get(
        "https://social-media-ankush.herokuapp.com/comments/get"
      );
  ...
  const handleCommentPost = async () => {
    try {
      const res = await axios.post(
        "https://social-media-ankush.herokuapp.com/comment/post",
        { comment: comment, postID: id, token: Cookies.get("jwt") }
      );
      ...
      <div className="comments">
        <div className="comments_left_block">
          ...
          <div className="comments_div">
            ...
            0 ? (
              <h2 className="no_comment_txt">
                No Comments yet , Being the first one 😎
              </h2>
            ) : (
              comments
          ...
          <div className="comment_post_wrapper">
            <div className="Post comment_post">
                ...
                <div className="post_header">
                  <div className="post_profile_img_wrapper">
                    <img
                      src={
                        postUser.profileImg
                          ? `https://social-media-ankush.herokuapp.com/public/Images/${postUser.profileImg}`
              ...
              <div className="post_body">
                <img
                  src={
                    commentPost.postImage
                      ? `https://social-media-ankush.herokuapp.com/public/Images/${commentPost.postImage}`
                      : `https://social-media-ankush.herokuapp.com/public/Images/user (1).png`
                  }
                  alt="profile"
                />
                  ...
                  <div className="icons">
                    <Link to={`/comments/${commentPost._id}`}>
                      <i
                        ...
                        onClick={() => {
                          handleLike(currentActiveUser._id, commentPost._id);
                        }}
                      ></i>
                    </Link>
                  ...
                  <Link to={`/comments/${commentPost._id}`}>
                    <p>
                      <span className="comment_count">
                        {
                          comments.filter(
                            (comment) => comment.postID === commentPost._id
                          ).length
                        }
                      </span>
                      comments
